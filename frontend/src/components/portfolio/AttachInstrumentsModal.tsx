...
const AttachInstrumentsModal: React.FC<AttachInstrumentsModalProps> = ({ portfolioId, onClose, onSuccess }) => {
  ...
  const loadAvailableBonds = async () => {
    try {
      setLoading(true);
      const bonds = await bondService.getAllBonds();
      ...
      <div className="bg-fd-darker rounded-lg shadow-xl border border-fd-border max-w-5xl w-full mx-4 max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-4 border-b border-fd-border">
          <div className="flex items-center space-x-4">
            <label className="text-sm font-medium text-fd-text">Instrument Type:</label>
            <div className="flex space-x-3">
              <button
                onClick={() => setInstrumentType('CDS')}
              ...
              <button
                onClick={() => setInstrumentType('BOND')}
                className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${
                  instrumentType === 'BOND'
                    ? 'bg-fd-green text-fd-dark'
                    : 'bg-fd-dark text-fd-text border border-fd-border hover:bg-fd-darker'
        ...
        {/* Weight Type (CDS only) */}
        {instrumentType === 'CDS' && (
          <div className="px-6 py-3 border-b border-fd-border">
            <div className="flex items-center space-x-4">
                    ...
                    <tr key={bond.id} className={`transition-colors ${isSelected ? 'bg-fd-dark ring-2 ring-fd-green' : 'hover:bg-fd-dark'}`}>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <input
                          type="checkbox"
                          checked={isSelected}
                          onChange={() => handleBondToggle(bond.id!)}
                          className="h-4 w-4 accent-fd-green border-fd-border rounded cursor-pointer"
                        />
                      </td>
...
export default AttachInstrumentsModal;
