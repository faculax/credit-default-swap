  ...
  return (
    ...
    >
      <div className="bg-fd-dark border border-fd-border rounded-md p-4 w-full max-w-xl">
        <h3 id="scenario-modal-title" className="text-fd-green font-semibold mb-2">
          Scenario Run
        </h3>
        <label className="block text-sm text-fd-text mb-2">
          Parallel bp shifts (comma separated)
          <input
            value={shifts}
            onChange={(e) => setShifts(e.target.value)}
            className="mt-1 w-full bg-fd-darker border border-fd-border rounded px-2 py-1 text-fd-text"
          />
        </label>
        <div className="flex gap-2 mb-4">
          <button
            onClick={submit}
            className="bg-fd-green text-fd-dark px-3 py-1 rounded disabled:opacity-50"
            disabled={loading}
          >
            Run
          </button>
          <button
            onClick={onClose}
            className="bg-fd-darker border border-fd-border text-fd-text px-3 py-1 rounded"
          >
            Close
          </button>
        </div>
              ...
              {result.scenarios.map((s) => (
                <tr key={s.scenario} className="border-t border-fd-border">
                  ...
                  <td className="py-1 font-mono">
                    {s.measures.fairSpreadClean
                      ? (s.measures.fairSpreadClean * 10000).toFixed(2)
                      : 'N/A'}
                  </td>
                </tr>
              ))}
