...
export const lifecycleService = {
  ...
  async getCouponScheduleInRange(
    tradeId: number,
    startDate: string,
    endDate: string
  ): Promise<CouponPeriod[]> {
    const params = new URLSearchParams({ startDate, endDate });
    const res = await fetch(
      `${API_BASE}/trades/${tradeId}/coupon-schedule/range?${params.toString()}`
    );
    return handleResponse<CouponPeriod[]>(res);
  },
  ...
  async postDailyAccrual(tradeId: number, accrualDate: string): Promise<AccrualEvent> {
    ...
    const res = await fetch(`${API_BASE}/trades/${tradeId}/accruals/daily?${params.toString()}`, {
      method: 'POST',
    });
    return handleResponse<AccrualEvent>(res);
  },
  async postAccrualsForPeriod(
    tradeId: number,
    startDate: string,
    endDate: string
  ): Promise<AccrualEvent[]> {
    const params = new URLSearchParams({ startDate, endDate });
    const res = await fetch(`${API_BASE}/trades/${tradeId}/accruals/period?${params.toString()}`, {
      method: 'POST',
    });
    return handleResponse<AccrualEvent[]>(res);
  },
  async getAccrualEvents(
    tradeId: number,
    startDate: string,
    endDate: string
  ): Promise<AccrualEvent[]> {
  ...
  async adjustNotional(
    tradeId: number,
    payload: NotionalAdjustmentPayload
  ): Promise<NotionalAdjustment> {
  ...
  async partiallyTerminate(
    ...
    const params = new URLSearchParams({
      terminationDate,
      terminationAmount: terminationAmount.toString(),
    });
    if (reason) params.append('reason', reason);
    const res = await fetch(
      `${API_BASE}/trades/${tradeId}/partial-termination?${params.toString()}`,
      { method: 'POST' }
    );
    return handleResponse<NotionalAdjustment>(res);
  },
  async fullyTerminate(
    ...
    if (reason) params.append('reason', reason);
    const res = await fetch(`${API_BASE}/trades/${tradeId}/full-termination?${params.toString()}`, {
      method: 'POST',
    });
    return handleResponse<NotionalAdjustment>(res);
  },
