...
param(
    ...
    [string]$ProjectRoot = "",
    [switch]$GroupByComponent = $false,  # Use tags instead of separate engagements
    [switch]$ReuseEngagement = $true,    # Reuse today's engagement if it exists
    [switch]$Verbose
)
...
if ($GroupByComponent) {
    ...
    # Single engagement mode with tag-based organization
    $today = Get-Date -Format "yyyy-MM-dd"
    $engagementId = $null
    
    # Try to find existing engagement for today if ReuseEngagement is enabled
    if ($ReuseEngagement) {
        Write-Step "Searching for existing engagement for today..."
        ...
        try {
            ...
            if ($existingEngagements.count -gt 0) {
                ...
                Write-Success "Reusing existing engagement (ID: $engagementId) - '$($existingEngagements.results[0].name)'"
                Write-Host "  💡 Scans will be reimported (existing findings will be updated)" -ForegroundColor Cyan
            }
        } catch {
            Write-Warning "Could not search for existing engagements: $($_.Exception.Message)"
        }
    }
    
    # Create new engagement if none found or ReuseEngagement is disabled
    if ($null -eq $engagementId) {
        Write-Step "Creating new engagement..."
...
function Upload-Scan {
    ...
    if (Test-Path $filePath) {
        Write-Step "Uploading $($Mapping.Description)..."
        Write-Host "  🏷️  Tag: $($Mapping.Tag)" -ForegroundColor Cyan
        ...
        try {
            ...
            $bodyLines = @(
                ...
                "true",
                "--$boundary",
                "Content-Disposition: form-data; name=`"close_old_findings`"",
                "",
                "true",
                "--$boundary",
                "Content-Disposition: form-data; name=`"test_title`"",
                "",
                "$($Mapping.Tag) - $($Mapping.Description)",
                "--$boundary",
...
Write-Host ""
Write-Host "🏷️  Filter by Component Tags in DefectDojo:" -ForegroundColor Yellow
Write-Host "   1. Go to: $DefectDojoUrl/engagement/$engagementId" -ForegroundColor White
Write-Host "   2. Click on 'Findings' tab" -ForegroundColor White
Write-Host "   3. Use the 'Tags' filter dropdown to select:" -ForegroundColor White
Write-Host "      • backend-code, backend-dependencies, backend-quality" -ForegroundColor Cyan
Write-Host "      • frontend-code, frontend-dependencies, frontend-libraries" -ForegroundColor Cyan
Write-Host "      • gateway-code, gateway-dependencies" -ForegroundColor Cyan
Write-Host "      • risk-engine-code, risk-engine-dependencies" -ForegroundColor Cyan
Write-Host ""
Write-Host "💡 Tip: You can also search by tag using the search bar" -ForegroundColor Yellow
Write-Host ""
